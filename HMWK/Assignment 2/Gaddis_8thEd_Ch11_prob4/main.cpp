...
int main(int argc, char** argv) {
    ...
    for(i=0;i<MONTHS-1;i++){       
        if(weather[i].tempL<weather[i+1].tempL){
            lowT=weather[i].tempL;
            monthL=i;
        }
        if (weather[i].tempL>weather[i+1].tempL){
        ...
        if(weather[i].tempH>weather[i+1].tempH){
            highT=weather[i].tempH;
            monthH=i;
        }
        if(weather[i].tempH<weather[i+1].tempH){
    ...
    cout <<"the highest temperature was "<< monthH <<", and that occurred"
            "in month #"<< monthH <<","<<endl;
    cout << "while the lowest was "<< monthL <<", and that occurred in month"
                    " #"<<monthL<<endl;
